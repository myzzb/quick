 ...
 */
package vip.xiaonuo.sys.modular.sgsj.service.impl;
...
import vip.xiaonuo.sys.modular.sgsj.mapper.SgsjMapper;
...
public class SgsjServiceImpl extends ServiceImpl<SgsjMapper, Sgsj> implements SgsjService {
    ...
    public Page<Sgsj> page(SgsjPageParam sgsjPageParam) {
        ...
        if(ObjectUtil.isNotEmpty(sgsjPageParam.getSglx())) {
            queryWrapper.lambda().like(Sgsj::getSglx, sgsjPageParam.getSglx());
        }
        if(ObjectUtil.isNotEmpty(sgsjPageParam.getSgmc())) {
            queryWrapper.lambda().like(Sgsj::getSgmc, sgsjPageParam.getSgmc());
        }
        if(ObjectUtil.isNotEmpty(sgsjPageParam.getIsAljx())) {
            queryWrapper.lambda().like(Sgsj::getIsAljx, sgsjPageParam.getIsAljx());
        }
    ...
    public Sgsj queryEntity(String id) {
        ...
        if(ObjectUtil.isEmpty(sgsj)) {
            throw new CommonException("事故数据信息表不存在，id值为：{}", id);
        }
        return sgsj;
    }
}
